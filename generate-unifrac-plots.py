from unifrac import faith_pd, weighted_normalized_fp64, unweighted_fp64_to_file, h5pcoa
...
# # table_wgs = table_wgs.subsample(500000)
...
# # # metadata = metadata.loc[metadata.index.isin(table_16s.ids())]
# # # metadata = metadata.rename(index=id_map)
# # # metadata.to_csv('16s-metadata.tsv', sep='\t', index=True)

# # # metadata_16s = pd.read_csv('16s-metadata.tsv', sep='\t', index_col=0)
# # # metadata_16s['type'] = '16s'
# # # metadata_wgs = pd.read_csv('wgs-metadata.tsv', sep='\t', index_col=0)
# # # metadata_wgs['type'] = 'wgs'

# # # pd.concat([metadata_16s, metadata_wgs]).to_csv('merged-metadata.tsv', sep='\t', index=True)
# # # # print(metadata.head())
# # # table_wgs.update_ids(id_map)
...
#     return table
# table_16s = min_feature_count(table_16s)
# table_16s = table_16s.subsample(5000)
# print('16s', table_16s.shape)
# table_wgs = min_feature_count(table_wgs)
# table_wgs = table_wgs.subsample(100000)
# print('wgs', table_wgs.shape)
...
# print(table.shape, np.max(table.pa(inplace=False).sum(axis='sample')))

# create input data
tree_path = 'unifrac-dataset/shared-trees/aligned-tree.nwk'
unifrac_path = unweighted_fp64_to_file(table_path, tree_path, 'test-unifrac.uni')
pcoa = h5pcoa('test-unifrac.uni')
pcoa.write('test-unifrac.txt')
# # s_ids = distance.ids
# # print(len(s_ids))
